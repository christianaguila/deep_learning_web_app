...
def uploadplant(request):
    ...
    if request.method == 'POST':
        ...
        if predict_form.is_valid():
            ...
            instance.save()
            prediction = ImageModel(f'uploads/{str(instance.plantimage)}')
            plant_image = instance.plantimage
            PredictedPlant.objects.create(prediction_label=prediction, post_prediction=instance)
            context = {'predict_form':predict_form, 'postsss': postsss, 'prediction': prediction, 'plant_image': plant_image}
